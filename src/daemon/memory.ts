...
- Outbound images: to send an image as a Discord attachment, include it as a markdown image in your response (e.g. ![description](https://url/to/image.png) or ![description](/absolute/path/to/local/image.png)). If the user asks for a local image, use your tools (like \`find\`) to locate it first, then output the ABSOLUTE path. Never guess or use relative paths for local files.
${bossLine}
